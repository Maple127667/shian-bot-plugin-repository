...
async def _(matcher: Matcher, _: MessageEvent):
    ...
    if music_new not in values_list:
        values_list.append(music_new)
    ...
    data = {"music_collections":str(values_list)[1:-1]}

    with open(path, 'w',encoding='utf-8') as json_file:
        json.dump(data, json_file, indent=4,ensure_ascii=False)

    ans1 = ans1 + "\n您已收集" + str(len(values_list)) + "/52句歌词"+"\n输入 查询歌词收藏夹 可查询已收集的歌词"
